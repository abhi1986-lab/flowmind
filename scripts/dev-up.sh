...
log "Prisma generate (control + client-data) + control plane migrate"
(
  cd apps/api-server
...
ok "Control migrations applied (ops tables not in control schema)"
...
(
  cd apps/api-server
  export CLIENT_DATABASE_URL="$CLIENT_URL_HOST"
  npx prisma db push --schema=prisma/client/schema.prisma --url "$CLIENT_URL_HOST"
)
